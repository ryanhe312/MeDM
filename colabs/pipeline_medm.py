...
class MeDMPipeline(DiffusionPipeline, TextualInversionLoaderMixin, LoraLoaderMixin):
    r"""
    ...
    def check_image(self, image, prompt, prompt_embeds):
        ...
        # TODO: allow input types other than tensor and allow batch size > 1
        if not image_is_tensor or len(image) != 1:
            raise ValueError('input image must be a torch tensor with batch dim == 1')
    ...
    def prepare_image(
        ...
        assert len(images) == 1 and batch_size == 1, 'batch size must be 1'
        images = images[0]

        images = self.control_image_processor.preprocess(images, height=height, width=width).to(dtype=torch.float32)

        # unstack video tensor into a list of image tensor (keeping batch dim)
        ...
        processed_images = processed_images.to(device=device, dtype=dtype) # MEMSAVE
    ...
    def prepare_latents(self, batch_size, num_channels_latents, height, width, dtype, device,
                        generator, image, timestep, vae_batch_size, latents=None):
        ...
        sh = image.shape
        init_latents = torch.zeros(sh[0], 4, sh[-2] // 8, sh[-1] // 8).to(device=device, dtype=dtype)
        # cast `pred_clean_latents_observed' to the latent space
        for frame_id in range(0, len(init_latents), vae_batch_size):
            init_latents[frame_id:frame_id+vae_batch_size] = self.vae.encode(
                image[frame_id:frame_id+vae_batch_size]).latent_dist.mode()
        init_latents *= self.vae.config.scaling_factor

        noise = latents.repeat_interleave(len(image), dim=0)
    ...
    def __call__(
        ...
        class Timer:
            ...
            def __exit__(self, _, __, ___):
                self.state[self.key] += time.time() - self.start
        ...
        # 3.5. HACK: offload text encoder
        if cpu_offload_text_encoder:
            self.text_encoder.cpu()

        # 4. Prepare image
        if original_image is not None:
            original_image = self.image_processor.preprocess(original_image[0]).to(dtype=torch.float32)
        ...
        timesteps, num_inference_steps = ((self.scheduler.timesteps, num_inference_steps) if original_image is None
                else self.get_timesteps(num_inference_steps, strength, device))
        ...
        images = image.to(prompt_embeds.dtype)
        latents_list = latents.repeat_interleave(len(images), dim=0) if original_image is None else latents
        pred_clean_latents_list = torch.zeros_like(latents_list)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                for frame_id in range(0, len(latents_list), unet_batch_size):
                    # image = image.to(device=device, dtype=controlnet.dtype) # MEMSAVE
                    image = images[frame_id:frame_id+unet_batch_size]
                    image = image.transpose(0, 1).flatten(0, 1) # cfg dim first, then flatten (cfg, batch)
                    latents = latents_list[frame_id:frame_id+unet_batch_size]

                    # expand the latents if we are doing classifier free guidance
                    latent_model_input = torch.cat([latents] * 2) if do_classifier_free_guidance else latents
                    latent_model_input = self.scheduler.scale_model_input(latent_model_input, t)

                    do_control = i < no_control_from_step
                    ...
                    if do_control:
                        ...
                        with Timer('controlnet'):
                            down_block_res_samples, mid_block_res_sample = self.controlnet(
                                control_model_input,
                                t,
                                encoder_hidden_states=controlnet_prompt_embeds.repeat_interleave(len(latents), dim=0),
                    ...
                    with Timer('unet'):
                        noise_pred = self.unet(
                            latent_model_input,
                            t,
                            encoder_hidden_states=prompt_embeds.repeat_interleave(len(latents), dim=0),
                            cross_attention_kwargs=cross_attention_kwargs,
                            down_block_additional_residuals=down_block_res_samples if do_control else None,
                            mid_block_additional_residual=mid_block_res_sample if do_control else None,
                            return_dict=False,
                        )[0]
                    ...
                    # we need to merge shared pixel in the predicted x_0 space
                    # so we derive x_0 here (noise -> x_0)
                    pred_clean_latents_list[frame_id:frame_id+unet_batch_size] = (
                            latents - beta_prod_t ** (0.5) * noise_pred) / alpha_prod_t ** (0.5) # MEMSAVE
                    latents_list[frame_id:frame_id+unet_batch_size] = latents


                ##################################################
                # NEW: mix the shared pixels
                if (i < min(no_mix_from_step, len(timesteps) - 1) # no mix in the last denoising step
                    and harmonization_scale > 0):
                    harmonized_pred_clean_latents_list = pred_clean_latents_list.clone()

                    # init the observed video tensor
                ...
                noise_pred = (latents_list - alpha_prod_t ** (0.5) * pred_clean_latents_list) / beta_prod_t ** (0.5)
                if (i < min(no_mix_from_step, len(timesteps) - 1) # no mix in the last denoising step
                    and harmonization_scale > 0):
                    noise_pred_harmonized = (latents_list - alpha_prod_t ** (0.5) *
                                             harmonized_pred_clean_latents_list) / beta_prod_t ** (0.5)
                    noise_pred = noise_pred + harmonization_scale * (noise_pred_harmonized - noise_pred)
                    
                # compute the previous noisy sample x_t -> x_t-1
                latents_list = self.scheduler.step(noise_pred, t, latents_list, **extra_step_kwargs,
                    return_dict=False)[0]
        ...
        if not output_type == "latent":
            ...
            with Timer('final_vae'):
                # decode latents
                latents /= self.vae.config.scaling_factor
                for frame_id in range(0, len(latents), vae_batch_size):
                    video[frame_id:frame_id+vae_batch_size] = self.vae.decode(
                        latents[frame_id:frame_id+vae_batch_size], return_dict=False)[0]
        ...
        # HACK: load the text encoder back the GPU
        if cpu_offload_text_encoder:
            self.text_encoder.to(device)

        ret = StableDiffusionPipelineOutput(images=image, nsfw_content_detected=has_nsfw_concept)
        ret.timer = Timer.state
        return ret
